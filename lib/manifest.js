...
module.exports = (api, pluginOptions, packageJson) => async (content) => {
  ...
  // CUSTOM_CHANGE:
  // content_security_policy changed to object in v3 instead of v2 string
  // manifest.content_security_policy =
  //   manifest.content_security_policy || "script-src 'self' 'unsafe-eval'; object-src 'self'"
  if(!manifest.content_security_policy) {
    logger.error('manifest.content_security_policy must be defined explicitly as object.');
    throw new Error('manifest.content_security_policy must be defined explicitly as object.');
  } else if(
    manifest.content_security_policy && 
    Object.keys(manifest.content_security_policy).length === 0
  ) {
    logger.error('Invalid manifest.content_security_policy for v3. It must be an object.');
    throw new Error('Invalid manifest.content_security_policy for v3. It must be an object.');
  } else if(
    !manifest.content_security_policy.extension_pages
  ) {
    logger.error('manifest.content_security_policy.extension_pages must be defined explicitly.');
    throw new Error('manifest.content_security_policy.extension_pages must be defined explicitly.');
  }
  ...
  if (isProduction) {
    // CUSTOM_CHANGE:
    // replace unsafe-eval for production
